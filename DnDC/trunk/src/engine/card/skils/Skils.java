 ...
 * @par TODO
 *  Dodanie wyjątku informującą że test jest niemożliwy. Miejsca w kodzie oznaczone FIXME.<br/>
...
public class Skils
{
    ...
    //Przyśpieszenie zapytania po nazwie umiejętności. Dodanie klucza powinno wyglądać put(CharacterSkil.getSkil.getName,CharacterSkill)

    public Skils(Character main)
    {
        this.main = main;
    }
     ...
     * @param skilToTest Umiejętnośc do testowania
     * @return Wynikt testu
     */
    private Integer testSkil(CharacterSkil skilToTest)
    {
        ...
        if(skilToTest.getSkil().isArmorInterrupt())
            return Dice.D20.throwTheDice() + skilToTest.getRank() + skilToTest.getBonus() + getAbilitiModifier(skilToTest.getSkil().getAbilitiModifier()) - main.getEq().getArmor().getArmorPenalty();
        return Dice.D20.throwTheDice() + skilToTest.getRank() + skilToTest.getBonus() + getAbilitiModifier(skilToTest.getSkil().getAbilitiModifier());
    }
